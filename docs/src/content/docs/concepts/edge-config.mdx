---
title: Edge Config
description: The shadow-configuration key schema — every field explained.
---

import { Aside } from '@astrojs/starlight/components';

Shadow-canary stores all routing state in a single Edge Config key. The middleware reads this key on every request (with a 60-second in-process cache). The three workflows write to it on every deploy or canary tick.

By default the key is `shadow-configuration`. Starting with `@dotworld/shadow-canary-core@0.2.0` you can override it per project via the `SHADOW_CANARY_KEY` environment variable — useful when sharing one Edge Config store across several projects (see [Sharing a store](#sharing-a-store-across-projects) below).

## Full schema

```json
{
  "deploymentDomainProd": "https://my-app-abc123.vercel.app",
  "deploymentDomainProdPrevious": "https://my-app-def456.vercel.app",
  "deploymentDomainShadow": "https://my-app-xyz789.vercel.app",
  "trafficShadowPercent": 1,
  "trafficProdCanaryPercent": 42,
  "shadowForceIPs": ["203.0.113.42"],
  "canaryPaused": false,
  "canaryStartedAt": "2024-11-15T08:00:00Z"
}
```

## Field reference

### `deploymentDomainProd`

| | |
|---|---|
| Type | `string` (URL) |
| Written by | `deploy-prod.yml` |
| Read by | Middleware (for internal bookkeeping), admin UI |
| Example | `https://my-app-abc123.vercel.app` |

The per-deployment URL of the current production deploy — the deploy that received `vercel promote`. This is the deploy where the middleware runs. It is stored here so the admin UI can display it without a Vercel API call.

### `deploymentDomainProdPrevious`

| | |
|---|---|
| Type | `string` (URL) or absent |
| Written by | `deploy-prod.yml` (sets on canary start, clears on `[skip-canary]`) |
| Read by | Middleware (rewrite target for `prod-previous` bucket) |
| Example | `https://my-app-def456.vercel.app` |

The per-deployment URL of the previous production deploy. Present during a canary and kept after it completes at 100%, so in-flight sessions on `prod-previous` can finish their journey. Overwritten by the next `deploy-prod.yml` run.

If this field is absent or empty, the middleware treats all prod traffic as `prod-new` (no canary in effect).

### `deploymentDomainShadow`

| | |
|---|---|
| Type | `string` (URL) |
| Written by | `deploy-shadow.yml` |
| Read by | Middleware (rewrite target for shadow bucket) |
| Example | `https://my-app-xyz789.vercel.app` |

The per-deployment URL of the current `master` deploy. Updated on every push to `master`. If this field is absent, shadow routing silently degrades — the middleware returns `NextResponse.next()`.

### `trafficShadowPercent`

| | |
|---|---|
| Type | `number` (0–100) |
| Default | `1` |
| Written by | `deploy-shadow.yml` (initializes to 1 if absent) |
| Read by | Middleware |

Percentage of requests routed to the shadow deploy. Typically `1`. Set to `0` as a kill-switch — the shadow deploy continues to exist but receives no traffic. Propagates within 60 seconds, no redeploy needed.

### `trafficProdCanaryPercent`

| | |
|---|---|
| Type | `number` (0–100) |
| Default | `100` (no canary) |
| Written by | `deploy-prod.yml` (resets on each deploy), `canary-ramp.yml` (increments) |
| Read by | Middleware |

Percentage of prod-bucket requests that go to the new deploy. The remainder goes to `deploymentDomainProdPrevious`.

| Value | Meaning |
|---|---|
| `100` | No active canary — all prod traffic on new deploy |
| `0` | Full rollback — all prod traffic on previous deploy |
| `1`–`99` | Canary in progress |

### `shadowForceIPs`

| | |
|---|---|
| Type | `string[]` (IPv4 addresses) |
| Default | `[]` |
| Written by | Manual (Vercel dashboard or API) |
| Read by | Middleware |

List of IPv4 addresses that are always routed to the shadow deploy, bypassing random roll and cookie stickiness. Used to route office or dev-team traffic to shadow permanently. No cookie is set for IP-forced requests.

<Aside type="caution">
CIDR ranges are not supported — only exact IPv4 addresses. If your office uses a dynamic IP, you will need to update this field when it changes.
</Aside>

### `canaryPaused`

| | |
|---|---|
| Type | `boolean` |
| Default | `false` |
| Written by | Admin UI (Pause/Resume buttons), `canary-ramp.yml` (sets `true` on SLO failure) |
| Read by | `canary-ramp.yml` |

When `true`, the canary cron skips the SLO check and bump. Traffic split is frozen at the current `trafficProdCanaryPercent`. Resume via the admin UI or by setting to `false` in Edge Config directly.

### `canaryStartedAt`

| | |
|---|---|
| Type | `string` (ISO 8601) or `null` |
| Written by | `deploy-prod.yml` (on canary start), `canary-ramp.yml` (clears at 100%) |
| Read by | Admin UI |

Timestamp of when the current canary started. Used for display purposes in the admin UI. `null` means no active canary.

## Cache TTL

The middleware caches the Edge Config response in module memory for 60 seconds. Config changes take up to 60 seconds to propagate to warm middleware instances. Cold starts (new instances) always fetch from Edge Config directly.

## Manual edits

You can edit the config directly in the Vercel dashboard (Storage > your store > Items). Common manual operations:

- Set `trafficShadowPercent: 0` to kill the shadow (shadow still exists, gets no traffic)
- Set `trafficProdCanaryPercent: 0` to roll back a canary manually
- Add or remove IPs in `shadowForceIPs`
- Set `canaryPaused: false` to resume a paused canary

## Sharing a store across projects

Vercel's Pro plan limits you to **3 Edge Config stores** per team. If you want to run shadow-canary on 4+ projects, you can host all their configs as separate keys inside one shared store.

### Setup

1. Link the same Edge Config store to every project (Vercel Storage → your store → Projects → Connect).
2. In each project's **Vercel environment variables** (Production + Preview), set:

   ```
   SHADOW_CANARY_KEY=shadow-configuration-<project-name>
   ```

   Use a unique value per project — e.g. `shadow-configuration-stargaze`, `shadow-configuration-checkout`, etc. This is read by the middleware and admin API at runtime.
3. In each project's **GitHub repo secrets** (Settings → Secrets and variables → Actions), set the exact same value:

   ```
   SHADOW_CANARY_KEY=shadow-configuration-<project-name>
   ```

   This is read by `deploy-shadow.yml`, `deploy-prod.yml`, and `canary-ramp.yml` when writing to Edge Config. **Forgetting this secret is the most common mistake when sharing a store** — the workflows fall back to the default `shadow-configuration` key and silently overwrite sibling projects' routing state on every deploy.
4. In the Edge Config store, create one item per project, each with its own key:

   ```json
   // Key: shadow-configuration-stargaze
   { "trafficShadowPercent": 0, "trafficProdCanaryPercent": 100, "shadowForceIPs": [] }

   // Key: shadow-configuration-checkout
   { "trafficShadowPercent": 1, "trafficProdCanaryPercent": 100, "shadowForceIPs": [] }
   ```

Runtime (middleware + admin API) reads the Vercel env var `SHADOW_CANARY_KEY`. Workflows read the GitHub Actions secret `SHADOW_CANARY_KEY`. Keep both values identical for a given project. No code changes in the host project.

<Aside type="caution">
Requires `@dotworld/shadow-canary-core` and `@dotworld/shadow-canary-templates` **≥ 0.2.1**. Versions 0.2.0 shipped the env var plumbing on the runtime side only — the workflows still hardcoded `shadow-configuration` and would overwrite neighbors.
</Aside>

### Programmatic override

If you want to target a specific key from code (not env), pass `configKey` explicitly:

```ts
import { shadowCanaryMiddleware } from '@dotworld/shadow-canary-core/edge';

export async function middleware(req: NextRequest) {
  return (await shadowCanaryMiddleware(req, {
    configKey: 'shadow-configuration-stargaze',
  })) ?? NextResponse.next();
}
```

<Aside type="caution">
Trade-offs: a single store with many keys centralizes all your routing state — a broken write or corrupt value affects every project on the same store. Keep stores dedicated when you can. Use the shared-store pattern deliberately, not by default.
</Aside>

### Limits inside one store

Edge Config has a **64 KB total payload limit per store**. A typical ShadowConfig item is ~400 bytes, so a single shared store can host ~150 projects in theory. Keep the `shadowForceIPs` list compact and you'll stay well under the limit.

---

**Related:**
- [Routing](/shadow-canary/concepts/routing/) — how the middleware uses this config
- [Workflows](/shadow-canary/reference/workflows/) — which fields each workflow writes
- [Dashboard](/shadow-canary/reference/dashboard/) — admin UI reading and writing this config
