...
4. Read the current `shadow-<repo-slug>-canary` entry from Edge Config
...
To change the shadow traffic percentage from 1%, edit the `shadow-<repo-slug>-canary` entry in the Edge Config directly — this workflow does not reset it. To permanently change the default, modify the `jq` merge expression in the workflow.
