# @shadow-canary/templates

File payload for [shadow-canary](https://github.com/your-org/shadow-canary) — copyable into existing Next.js + Vercel projects.

This package ships the host-side files that must live in the project repository. Logic is imported from `@shadow-canary/core`; these files are thin wrappers, UI components, API routes, and CI workflows.
...
```bash
npx @shadow-canary/templates copy . --force
```

You can also target a specific directory:

```bash
npx @shadow-canary/templates copy /path/to/my-next-app
```
...
| Path | Purpose |
|---|---|
| `middleware.ts` | Edge middleware: routes shadow/canary traffic via `@shadow-canary/core/edge` |
...
| `lib/admin-vercel.ts` | Vercel API helpers: thin re-exports from `@shadow-canary/core` |
...
1. Install dependencies (`@shadow-canary/core`, `@vercel/edge-config`)
